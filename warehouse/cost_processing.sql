...
BEGIN
    ...
    BEGIN
        ...
        SELECT 
            ...
            -- Modificar para extraer el Proyecto desde la columna Tags con las diferentes variantes posibles --
            CASE
                WHEN JSON_VALUE(CONCAT('{', [Tags], ' }'), '$.Application_Name') IS NOT NULL THEN JSON_VALUE(CONCAT('{', [Tags], ' }'), '$.Application_Name')
                WHEN JSON_VALUE(CONCAT('{', [Tags], ' }'), '$." Application_Name"') IS NOT NULL THEN JSON_VALUE(CONCAT('{', [Tags], ' }'), '$." Application_Name"')
                WHEN JSON_VALUE(CONCAT('{', [Tags], ' }'), '$."Application_Name "') IS NOT NULL THEN JSON_VALUE(CONCAT('{', [Tags], ' }'), '$."Application_Name "')
                WHEN JSON_VALUE(CONCAT('{', [Tags], ' }'), '$."Application Name"') IS NOT NULL THEN JSON_VALUE(CONCAT('{', [Tags], ' }'), '$."Application Name"')
                ELSE JSON_VALUE(CONCAT('{', [Tags], ' }'), '$."DnA Program"')
            END AS [Proyecto],
        ...
        GROUP BY [Date],[SubscriptionId],[SubscriptionName],[ResourceLocation],[UnitOfMeasure],[UnitOfMeasureNumeric],[ResourceGroup],
                 ...
                 JSON_VALUE(CONCAT('{', [Tags], ' }'), '$.Application_Name'),
                 JSON_VALUE(CONCAT('{', [Tags], ' }'), '$." Application_Name"'),
                 JSON_VALUE(CONCAT('{', [Tags], ' }'), '$."Application_Name "'),
                 JSON_VALUE(CONCAT('{', [Tags], ' }'), '$."Application Name"'),
                 JSON_VALUE(CONCAT('{', [Tags], ' }'), '$."DnA Program"'),
                 JSON_VALUE(CONCAT('{', [Tags], ' }'), '$.environment');
